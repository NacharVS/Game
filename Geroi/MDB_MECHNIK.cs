...
namespace Geroi
{
    class Unit2
    {
        ...
        private string v4;
        private string v5;
        public string Path => path_mechnik;

        public Unit2(int v1, int v2, int v3, string v4, string v5)
        {
            this.v1 = v1;
            this.v2 = v2;
            this.v3 = v3;
        ...
        [BsonElement]
        public string _image { get => Path; set => path_mechnik = value; }
        ...
        public void Add(Unit2 unit)
        {
            ...
            var b = abase.GetCollection<Unit2>("Shakirov_DB");
            b.InsertOne(unit);
        }
        public static void FindFrom(Unit2 unit)
        {
            MongoClient client = new MongoClient();
            var abase = client.GetDatabase("111");
            var b = abase.GetCollection<Unit2>("Shakirov_DB");
            var listPerson = b.Find(Geroi => unit._name == "ovosh").ToList();
            foreach (Unit2 unit1 in listPerson)
            {
